...
#Lo visto en la U4 y U5
#De aqui en adelante continuaremos el ejercicio, esto constituye la U6

#Selección de Variables

#Cargamos en memoria el paquete leaps
install.packages('leaps')
library(leaps)

regbest.full <- regsubsets(logRev_Total~.-CD-MM-Savings,data = train,nvmax = 13)
...
points(4,reg.summary$bi[4],col="red",cex=2,pch=20)

coef(regbest.full,4)

#Selección de variables paso a paso:
nulo <- lm(logRev_Total~1,data=train)
nulo

completo <- lm(logRev_Total ~.-CD-MM-Savings,data=train)
completo

step(nulo, scope=list(lower=nulo, upper=completo), direcFon="forward")
step(completo, data=train, direcFon="backward")
step(nulo, scope = list(upper=completo), data=train, direcFon="both")

#Ajustamos el modelo final
modelo.final <- lm(formula = logRev_Total ~ logBal_Total + CARD + Check +
                     Offer + LOAN + INSUR, data = train)
modelo.final

#Métricas

#Cargamos en memoria el paquete metrics
install.packages('Metrics')
library(Metrics)

#Data de entrenamiento
mae(train$logRev_Total,predict(modelo.final))
rmse(train$logRev_Total,predict(modelo.final))

#Data de testing
mae(test$logRev_Total,predict(modelo.final, newdata=test))
rmse(test$logRev_Total,predict(modelo.final, newdata=test))

#Cargamos en memoria el paquete metrics
install.packages('MPV')
library(MPV)

#Calculamos la métrica RMES usando el método de validación cruzada dejando uno afuera
PRESS(modelo.final)

3941.582/nrow(train)

sqrt(3941.582/nrow(train))
